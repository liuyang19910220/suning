$(function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      // console.log(data[0]);//{id: "10001", title: "苹果(Apple) iPhone 6 32GB 金色 移动联通电信 全网通4G手机", price: "￥2999.00", src: Array(4)}
      //2成功后,取出localStorage中存储的商品
      var str = localStorage.getItem("cart");
      if (str) {
        var obj = JSON.parse(str);
        // console.log(obj);//{10001: 1, 10002: 5, 10004: 1}
        //3动态创建商品列表
        var html = "";
        //cookie里的i对应商品id,同时对应list.json中的id属性
        for (var i in obj) {
          for (var j = 0; j < data.length; j++) {
            if (i == data[j].id) {
              // i = j;
              // console.log(j, i,data[i].price);
              var perPrice = (data[j].price).replace("￥", "");
              html += "<li><label class='selectItem'></label><img src='" +
                data[j].src[0] +
                "'/><p class='title'>" + i + data[j].title +
                "</p><span class='price'>" + data[j].price +
          ...
          for (var i in obj) {
            for (var j = 0; j < data.length; j++) {
              if (i == data[j].id) {
                ...
                totalAll += obj[i] * ((data[j].price).replace("￥", ""));
              }
            }
